 ...
 */
contract ETHOption is Initializable, ERC20Detailed, ERC20 {
    ...
    /**
     * The strike asset for this vault
     */
    IWETH public strikeAsset;
    ...
    function mint() external payable beforeExpiration {
        ...
        strikeAsset.deposit.value(amount)();
    }
    ...
    function burn(uint256 amount) external beforeExpiration {
        ...
        require(amount <= lockedBalance[recipient], "Not enough underlying balance");

        // Burn option tokens
        lockedBalance[msg.sender] = lockedBalance[recipient].sub(amount);
        _burn(recipient, amount);

        // Unlocks the strike token
        strikeAsset.withdraw(amount);
    ...
    function exchange(uint256 amount) external beforeExpiration {
        ...
        strikeAsset.withdraw(amount.mul(strikePrice), msg.sender);
    }
...
//            require(strikeAsset.transfer(msg.sender, strikeToReceive), "Couldn't transfer back strike tokens to caller");
            strikeAsset.withdraw(strikeToReceive, msg.sender);
        }
